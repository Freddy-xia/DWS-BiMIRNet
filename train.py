...
import argparse
from losses import DTCWTMagLoss
...
parser.add_argument('--train_dir', default='F:\\NeRD-Rain-main\Datasets\Rain200L\\train', type=str, help='Directory of train images')
parser.add_argument('--val_dir', default='F:\\NeRD-Rain-main\Datasets\Rain200L\\test', type=str, help='Directory of validation images')
parser.add_argument('--model_save_dir', default='F:\\NeRD-Rain-main\checkpoints', type=str, help='Path to save weights')
...
def laplacian_3x3(x: torch.Tensor):
    # x: [B,3,H,W] float
    # kernel: 4-neighbor Laplacian
    k = x.new_tensor([[0, 1, 0],
                      [1,-4, 1],
                      [0, 1, 0]], dtype=x.dtype)
    k = k.view(1, 1, 3, 3).repeat(3, 1, 1, 1)   # groups=3
    # replicate pad 更稳（避免边界伪影）
    x_pad = F.pad(x, (1,1,1,1), mode='reflect')
    return F.conv2d(x_pad, k, groups=3)
...
# criterion_dtcwt = DTCWTMagLoss(J=2, w_low=0.2, w_high=1.0, use_log1p=True).cuda()
...
iter = 0
criterion_dtcwt = DTCWTMagLoss(J=3).cuda()  # J=3 很适配 256 patch
lambda_dtcwt = 0.05  # 起步系数，后面可调 0.02/0.05
...
for epoch in range(start_epoch, num_epochs + 1):
    ...
    for i, data in enumerate(tqdm(train_loader), 0):
        ...
        M_max, M_mid, M_small = model_restoration._last_masks
        # 用 detach 的 mask 做约束，避免 mask 学成全 1 来逃避惩罚
        M_max = M_max.detach()
        M_mid = M_mid.detach()
        M_small = M_small.detach()

        inp_max = input_
        inp_mid = F.interpolate(input_, scale_factor=0.5, mode='bilinear', align_corners=False)
        inp_small = F.interpolate(input_, scale_factor=0.25, mode='bilinear', align_corners=False)

        # restored[0]=max, restored[1]=mid, restored[2]=small（按你现在 loss 写法）
        M_max_d = M_max.detach()
        M_mid_d = M_mid.detach()
        M_small_d = M_small.detach()

        # 变成 3 通道 mask
        Mm = M_max_d.repeat(1, 3, 1, 1)
        Md = M_mid_d.repeat(1, 3, 1, 1)
        Ms = M_small_d.repeat(1, 3, 1, 1)

        lap_r0 = laplacian_3x3(restored[0])
        lap_i0 = laplacian_3x3(inp_max)
        lap_r1 = laplacian_3x3(restored[1])
        lap_i1 = laplacian_3x3(inp_mid)
        lap_r2 = laplacian_3x3(restored[2])
        lap_i2 = laplacian_3x3(inp_small)

        loss_keep = (torch.abs((1 - Mm) * (lap_r0 - lap_i0)).mean() +
                     torch.abs((1 - Md) * (lap_r1 - lap_i1)).mean() +
                     torch.abs((1 - Ms) * (lap_r2 - lap_i2)).mean())

        loss_sparse = (M_max.mean() + M_mid.mean() + M_small.mean())
        w_keep = 0.05  # 建议 0.02~0.1 搜
        w_sparse = 1e-3  # 建议 1e-4~5e-3 搜
        ...
        loss_dtcwt = (criterion_dtcwt(restored[0], target[0]) +criterion_dtcwt(restored[1], target[1]) +criterion_dtcwt(restored[2], target[2]))

        loss = loss_char + 0.01 * loss_fft + 0.05 * loss_edge + 0.1 * loss_l1 + w_keep*loss_keep + w_sparse*loss_sparse + lambda_dtcwt * loss_dtcwt
        loss.backward()
        optimizer.step()
        ...
        writer.add_scalar('loss/dtcwt_loss', loss_dtcwt.item(), iter)
        writer.add_scalar('loss/keep_loss', loss_keep, iter)
        writer.add_scalar('loss/sparse_loss', loss_sparse, iter)
